...
result=$(curl -s "localhost:80/server_files/src/login.php")
...
result=$(curl -s "localhost:80/server_files/src/login.php?user=nonexistent&pass=invalid")
...
result=$(curl -s "localhost:80/server_files/src/login.php?user=apple&pass=apple")
